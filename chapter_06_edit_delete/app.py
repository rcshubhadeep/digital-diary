...
from db import delete_entry, edit_entry
...
@app.route("/delete/<diary_id>")
def delete(diary_id=None):
    '''We will delete the entry here, and return the status of the same.'''
    if delete_entry(diary_id) == True:
...
def edit():
    if not request.json:
        return json.dumps({"result": "error"})
    print request.json
    if edit_entry(request.json['id'], request.json['text']) == True:
        return json.dumps({"result": "success"})
    else:
        return json.dumps({"result": "error"})
